...
Independent copy of algo_v2/main.py (the XAUUSD V2 bot). Timeframe roles
were swapped from the original M5-anchor design per spec:

  - M15 is now the zone anchor (ATR + M15's own OB flips define the
    ...
    boundary (that's what actually flips the bias). M5 is now the
    subordinate STRICT tier: it only trades when its direction ALSO
    matches M15/the zone's effective direction right now -- no "postdates
    the boundary" exception, so it never fires against the current bias.
    This is the opposite pairing from algo_v2 (XAUUSD), where M5 is the
    anchor and M1/M3 are the strict tiers above it -- same mechanism,
    different timeframe playing the anchor role. M30 is meant to be added
    as a further strict tier once this two-tier version is validated live.
  - The ATR Trail input to the zone race is ALSO M15-based now, not M5 --
    it has to match the OB-flip inputs' timeframe for the "most recent of
    the three" race to mean anything (see zone.py's docstring). This means
    the indicator must be attached to the M15 USOIL chart (not M5) so it
    publishes ATRSTATE_USOIL_15.json; algo_v2_usoil no longer needs
    ATRSTATE_USOIL_5.json at all.
  ...
  - The forces-a-close rule (fresh_opposite_ob_exists) now checks M15 (the
...
def _direction_edges(direction: int, m5: Optional[OBSnapshot], m15: Optional[OBSnapshot]) -> dict:
...
def run_once(cfg: Config, store: TradedZoneStore, blocked: BlockedZoneStore, runtime: RuntimeState) -> None:
    ...
    # 2. Trail every open position in its own direction. M5 and M15's OB
    #    edges both feed SL selection, regardless of which tier opened the
    #    trade (matches algo_v2's M15/M5/M3 pattern).
    for pos in broker.get_positions(cfg.symbol, cfg.magic_number):
        direction = 1 if pos.type == mt5.POSITION_TYPE_BUY else -1
        edges = _direction_edges(direction, m5, m15)
