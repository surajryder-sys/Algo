...
Usage:
    ...
    python -m algo_v2_usoil.reset_block all
"""
from __future__ import annotations

import sys

from algo_v2_usoil.blocking import BlockedZoneStore
from algo_v2_usoil.config import load_config
...
def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in VALID:
        print(f"Usage: python -m algo_v2_usoil.reset_block <{'|'.join(VALID)}>")
        raise SystemExit(1)

    cfg = load_config()
    store = BlockedZoneStore(cfg.blocked_state_file)

    targets = ["M5", "M15"] if sys.argv[1] == "all" else [sys.argv[1]]
    for tf in targets:
