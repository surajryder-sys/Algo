...
gets its OWN tracker instance (V7S's symbol-agnostic-instance pattern,
...
from v7_sentinel import bridge, flip_state
...
class StaleAlertTracker:
    ...
    def check(self, tf_minutes: int, available: bool) -> Optional[str]:
        ...
        if not self._alerted.get(tf_minutes) and (now - since) > STALE_ALERT_THRESHOLD_SECONDS:
            ...
            return (f"[V7S-STR-ALERT] M{tf_minutes} bridge data has been stale/missing for over "
