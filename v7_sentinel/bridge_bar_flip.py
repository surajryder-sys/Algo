...
BRIDGE's line VALUES, not copy_rates' own computed trail lines. Ported
from v5_sentinel/bridge_bar_flip.py (2026-09-18) -- keeps the exact same
"only ever act on a closed bar" philosophy the rest of this system
already uses, but the trail line VALUES tested against each new closed
bar's close come from the bridge, not an independent copy_rates
recompute (removes the drift-prone recompute as a dependency entirely
for M3/M5/M15's own state).
...
every caller keeps working against the exact same type -- only WHERE
that value comes from has changed.

Multi-instrument note (2026-09-18, V7S): BridgeBarFlipTracker's
_state dict is keyed by tf_minutes ONLY, not (symbol, tf_minutes) --
same as V5S. This is fine ONLY because each symbol gets its OWN tracker
instance with its OWN state file (V7S's symbol-agnostic-instance
pattern + one-state-file-per-symbol decision, see config.py's
state_file_for()), never one tracker instance/file shared across
symbols. A caller wiring this up for multiple symbols must instantiate
one BridgeBarFlipTracker per symbol, each pointed at
config.state_file_for("bridge_bar_flip_state", symbol).
"""
...
from v7_sentinel import bridge
from v7_sentinel.flip_state import Confirmed, EventType, FlipEvent, FlipStateResult, far_near_line
from v7_sentinel.rates import _TIMEFRAME_CONST
...
class _PersistedState:
    ...
    # Deliberately SEPARATE from FlipStateResult.far_line/near_line
    # (_to_result() below), which stay a live re-read on every call on
    # purpose (SL Manager's ongoing post-breakeven trailing needs that to
    # keep moving, not freeze). Use BridgeBarFlipTracker.event_far_near()
    # to read these.
...
class BridgeBarFlipTracker:
    ...
    see module docstring's multi-instrument note: one instance per
    symbol, each with its own state file, is what makes this safe)."""
    ...
    def update(self, symbol: str, tf_minutes: int) -> Optional[FlipStateResult]:
        ...
        -- existing bar-time dedup keeps working unchanged). None only
        if there's no baseline at all yet (bar data itself unavailable,
        or the very first bar's bridge data was never available
        either)."""
