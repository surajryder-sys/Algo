"""Shared configuration for V7-Sentinel's data/bridge layer, loaded from
environment variables (.env). Fully independent of every other bot in
this repo -- own V7S_-prefixed env vars, own state files.

Multi-instrument from day one (see v7_sentinel/__init__.py): ACTIVE_SYMBOLS
...
internally) -- e.g. v7s_reversal_manager_state_XAUUSD.json. Only XAUUSD is
...
# Every symbol V7-Sentinel currently serves. XAUUSD only for now --
# extending this list is the intended way to add an instrument later.
ACTIVE_SYMBOLS: list[str] = [
    s.strip() for s in os.getenv("V7S_SYMBOLS", "XAUUSD").split(",") if s.strip()
]

POLL_SECONDS: float = float(os.getenv("V7S_POLL_SECONDS", "1"))
