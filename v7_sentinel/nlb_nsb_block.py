...
Reads the V7S Scraper's output file (v7_sentinel/tv_scraper), read-only.
(Until 2026-09-21 this read V5S's scraper output; the scraper was renamed/
ported to V7S so V5S can be retired.) V7S needs that scraper process
...
update) -- a standalone process (V7S's own nlb_nsb_watcher, not yet
...
from v7_sentinel import ob_levels
...
# validate TradingView-sourced zones against). H2 (2026-09-18, V7S's own
                ...
                for z in raw.get(key, {}).values():
                    ...
                    if not _is_aligned_to_timeframe(start_time, tf):
                        ...
                        self._announce_rejection(
                            ...
                            f"[V7S-BLOCK] zone {symbol}|{tf}|{direction}|{start_time} "
