...
M3/M5 CISD exit) were removed entirely 2026-09-25 along with Scalper itself.

THREE COMPONENTS (user's own split, 2026-09-21/22/24):
  ...
  3. ICT EXIT (exit_manager_ict.py, built 2026-09-24) -- an OB zone (M3/M5 from the merged
...
from v7_sentinel import broker, config, exit_manager_bias, exit_manager_ict, \
...
def run_once(rt: _SymbolRuntime) -> None:
    ...
    exit_manager_ict.run_once(rt.cfg, rt.ict)                     # component 3: ICT Exit
...
def main() -> None:
    ...
    for c in cfgs:
        ...
        print(f"[V7S-XM] {c.symbol} starting -- enable_trading={c.enable_trading} poll={c.poll_seconds}s "
              f"watching={[s.name for s in c.sources]} components=[bias, ltf, ict] "
