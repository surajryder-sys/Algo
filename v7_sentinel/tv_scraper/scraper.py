...
its Data Window panel for the current OB-zone state. Feeds V7-Sentinel's
second Reversal Manager component (OB-zone based, alongside the existing
STR/ATR-trail one).

Run with: python -m v7_sentinel.tv_scraper.scraper
...
saved into V7S_TV_SCRAPER_PROFILE_DIR and reused on every future run -- you
only log in once.
...
from v7_sentinel import heartbeat
from v7_sentinel.tv_scraper.config import Config, load_config
from v7_sentinel.tv_scraper.first_seen_store import FirstSeenStore
from v7_sentinel.tv_scraper.live_snapshot_store import LiveSnapshotStore
from v7_sentinel.tv_scraper.mitigation_track_store import MitigationTrackStore
from v7_sentinel.tv_scraper.parser import parse_data_window
from v7_sentinel.tv_scraper.retest_tracker import RetestTracker
from v7_sentinel.tv_scraper import zone_history_log
from v7_sentinel.tv_scraper.zone_store import TVZone, ZoneStore
...
def _goto_resilient(page: Page, url: str, attempts: int = 4) -> None:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if attempt == attempts:
                raise
            print(f"[V7S-TVZ] goto interrupted (attempt {attempt}/{attempts}), retrying: {exc}")
            time.sleep(2)
...
def _ensure_logged_in(page: Page, chart_url: str) -> None:
    ...
    print("[V7S-TVZ] Not logged in. Log into TradingView in the opened "
          "browser window, then come back here.")
    _goto_resilient(page, "https://www.tradingview.com/")
    page.wait_for_load_state("load")
    input("[V7S-TVZ] Press Enter once you're logged in... ")
...
def _apply_direction(zones: ZoneStore, first_seen: FirstSeenStore, retested: RetestTracker,
    ...
    for stored_zone in zones.zones(symbol, timeframe, direction):
        ...
        print(f"[V7S-TVZ] {symbol} {timeframe} {direction}: deleted orphaned zone "
    ...
    for zone in current:
        if not _price_plausible(symbol, zone["top"], zone["btm"]):
            print(f"[V7S-TVZ] {symbol} {timeframe} {direction}: REJECTED implausible zone "
...
def run_once_pane(page: Page, zones: ZoneStore, first_seen: FirstSeenStore,
    ...
    if parsed.atr is None and not parsed.bull_zones and not parsed.bear_zones:
        print(f"[V7S-TVZ][{pane_label}][DEBUG] captured {len(text)} chars | "
    ...
    if parsed.symbol is None:
        print(f"[V7S-TVZ][{pane_label}] no symbol detected (pane empty?) -- skipping")
        return
    ...
    if last_symbol_tf is not None and last_symbol_tf != (symbol, timeframe):
        print(f"[V7S-TVZ][{pane_label}] symbol/timeframe changed "
    ...
    if _last_processed_pane is not None:
        ...
        if prev_label != pane_label and prev_signature == this_signature:
            print(f"[V7S-TVZ][{pane_label}] {symbol} {timeframe}: identical to {prev_label}'s just-read "
    ...
    if parsed.close is not None and not _price_plausible(symbol, parsed.close, parsed.close):
        print(f"[V7S-TVZ] {symbol} {timeframe}: REJECTED implausible Close={parsed.close} "
    ...
    print(f"[V7S-TVZ][{pane_label}] symbol={symbol} tf={timeframe} "
...
def _connect_browser(p, cfg: Config) -> Browser:
    ...
    if not _cdp_port_open(cfg.cdp_port):
        print(f"[V7S-TVZ] no browser on CDP port {cfg.cdp_port} -- launching one")
        ...
        for _ in range(30):
            ...
            raise RuntimeError(
                ...
                f"check V7S_TV_SCRAPER_BROWSER_PATH points at a real Brave/Chrome install."
            )
    else:
        print(f"[V7S-TVZ] attaching to existing browser on CDP port {cfg.cdp_port}")
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        print(f"[V7S-TVZ] grid {cfg.grid_rows}x{cfg.grid_cols} -> panes: "
