...
Comments: "V7S-TM-STR-{trigger}" on entry (e.g. "V7S-TM-STR-M5CD"),
...
V7S_TM_{SYMBOL}_ENABLE_TRADING) must be explicitly true for any order to
...
from v7_sentinel import bridge, bridge_flip, broker, cisd_bridge, config, decision_log, flip_state, heartbeat, position_size_manager, rates, session_manager, sideways_trapper, sl_manager, telegram_alerts, trade_journal, trade_manager, trend_bias, trend_entry
from v7_sentinel.bridge_bar_flip import BridgeBarFlipTracker
from v7_sentinel.flip_state import far_near_line
from v7_sentinel.trend_config import TMSymbolConfig, load_symbol_config
...
def _entry_comment(tag: str) -> str:
    return f"V7S-TM-STR-{tag}"


def _action_comment(tag: str, action_code: str) -> str:
    return f"V7S-TM-STR-{tag}-{action_code}"


def _extract_tag(comment: str) -> str:
    """The entry tag out of a "V7S-TM-STR-{tag}[-action]" comment, used to
    ...
    if len(parts) >= 4 and parts[0] == "V7S" and parts[1] == "TM" and parts[2] == "STR":
        return "-".join(parts[3:])
    return "UNK"
...
def _open_position(cfg: TMSymbolConfig, direction: int, sl: float, tag: str, ref_desc: str,
    ...
    print(f"[V7S-TM-ENTRY] {cfg.symbol} {_DIR_LABEL[direction]} ({tag}) {ref_desc} sl={sl:.3f} lots={lots}")
    if not cfg.enable_trading:
        print("[V7S-TM-ENTRY] enable_trading is false -- decision only, no order sent")
    ...
    if not result.ok:
        print(f"[V7S-TM-ENTRY] order_send failed: retcode={result.retcode} comment={result.comment}")
        ...
        return False
    print(f"[V7S-TM-ENTRY] filled, ticket={result.ticket}")
    ...
    telegram_alerts.send_if_configured(
        ...
        f"[V7S] ENTRY: TM-STR {_DIR_LABEL[direction]} {cfg.symbol} #{result.ticket} ({tag}) -- "
    ...
    if event_time is not None and tf:
        ...
        if lag > ENTRY_BRIDGE_LAG_ALERT_SECONDS:
            telegram_alerts.send_if_configured(
                cfg.alerts_bot_token, cfg.alerts_chat_id,
                f"[V7S] BRIDGE LAG: TM-STR {_DIR_LABEL[direction]} {cfg.symbol} #{result.ticket} -- "
...
def _close_position(cfg: TMSymbolConfig, position, action_label: str, tag: str, action_code: str,
    ...
    print(f"[V7S-TM-EXIT] {cfg.symbol} closing #{position.ticket} ({action_label}), volume={position.volume}")
    if not cfg.enable_trading:
        print("[V7S-TM-EXIT] enable_trading is false -- decision only, no order sent")
    ...
    if not result.ok:
        print(f"[V7S-TM-EXIT] close failed: retcode={result.retcode} comment={result.comment}")
...
def _run_sl_manager(cfg: TMSymbolConfig, mgr: sl_manager.SLManager, tm_mgr: trade_manager.TradeManager, position,
    ...
    if far is None:
        print(f"[V7S-TM-SL] M{cfg.trailing_timeframe} far line unavailable -- skipping SL update this cycle")
        return
    ...
    if proposed is None:
        return
    print(f"[V7S-TM-SL] #{position.ticket} -> {proposed:.3f}")
    if not cfg.enable_trading:
        print("[V7S-TM-SL] enable_trading is false -- decision only, no modify sent")
        return
    ...
    if result.ok:
        ...
        print(f"[V7S-TM-SL] modify failed: retcode={result.retcode} comment={result.comment}")
...
def _run_trade_manager(cfg: TMSymbolConfig, mgr: trade_manager.TradeManager, position,
    ...
    if position_size_manager.is_night():
        ...
        print(f"[V7S-TM-TM] #{position.ticket} NIGHT MODE full close -- "
    ...
    print(f"[V7S-TM-TM] #{position.ticket} booking {label}: {volume} lots")
    if not cfg.enable_trading:
        print("[V7S-TM-TM] enable_trading is false -- decision only, no close sent")
        return
    ...
    if not result.ok:
        print(f"[V7S-TM-TM] partial close failed: retcode={result.retcode} comment={result.comment}")
...
def run_once(rt: _SymbolRuntime) -> None:
    ...
    if feed_event == "stale":
        msg = (f"[V7S-TM] {cfg.symbol} M{cfg.bias_timeframe} ATR/CISD bridge has been stale for over "
        ...
        msg = f"[V7S-TM] {cfg.symbol} M{cfg.bias_timeframe} ATR/CISD bridge is fresh again -- new entries resumed."
        print(msg)
    ...
    if state_event == "stale":
        msg = (f"[V7S-TM] {cfg.symbol} M{cfg.squareoff_timeframe} ATR bridge has been stale for over "
        ...
        msg = f"[V7S-TM] {cfg.symbol} M{cfg.squareoff_timeframe} ATR bridge is fresh again -- M5 exits resumed."
        print(msg)
    ...
    if rt.journal is not None:
        ...
        for exit_rec in tm_exits:
            if exit_rec.get("profit_net", 0) < 0 and rt.trapper is not None:
                ...
                print(f"[V7S-TM] Sideways Trapper recorded {exit_rec['direction']} loss "
    ...
    if position is not None and not m5_exits_paused:
        ...
        if flip is not None:
            ...
            print(f"[V7S-TM] {cfg.symbol} M{cfg.squareoff_timeframe} state flipped {flip.confirmed.name} "
    ...
    if position is not None and not m5_exits_paused:
        ...
        if cisd is not None:
            tag = _extract_tag(rt.tm_mgr.get_entry_comment(position.ticket) or position.comment or "")
            print(f"[V7S-TM] {cfg.symbol} M{cfg.squareoff_timeframe} {cisd.last_cisd} CISD@{cisd.last_cisd_time} "
    ...
    if session_event == "entered":
        msg = f"[V7S-TM] {cfg.symbol} entering session window '{session_window}' -- new entries PAUSED until it ends (open trades still managed)."
        print(msg)
        telegram_alerts.send_if_configured(cfg.alerts_bot_token, cfg.alerts_chat_id, msg)
    elif session_event == "left":
        msg = f"[V7S-TM] {cfg.symbol} session window ended -- new entries resumed."
        print(msg)
        telegram_alerts.send_if_configured(cfg.alerts_bot_token, cfg.alerts_chat_id, msg)
        ...
        if sig is not None:
            ...
            if position is None:
                ...
                if sig.direction == pos_direction:
                    rt.eligibility.mark_traded(sig.timeframe_minutes, sig.event_time)
                    print(f"[V7S-TM] {sig.trigger} qualifies ({_DIR_LABEL[sig.direction]}) but a "
                    ...
                    telegram_alerts.send_if_configured(
                        cfg.alerts_bot_token, cfg.alerts_chat_id,
                        f"[V7S] SKIPPED: TM-STR {_DIR_LABEL[sig.direction]} {cfg.symbol} ({sig.trigger}) -- "
                    ...
                    print(f"[V7S-TM] {sig.trigger} qualifies ({_DIR_LABEL[sig.direction]}) but an opposite "
    ...
    if rt.trapper is not None and rt.trapper.last_block_reason is not None:
        msg = f"[V7S-TM] {cfg.symbol} Sideways Trapper blocked entry -- {rt.trapper.last_block_reason}"
        print(msg)
...
def main() -> None:
    ...
    for rt in runtimes:
        cfg = rt.cfg
        print(f"[V7S-TM] {cfg.symbol} starting -- magic={cfg.magic_number} bias=M{cfg.bias_timeframe} "
        ...
        while True:
            for rt in runtimes:
                try:
                    ...
                    print(f"[V7S-TM] {rt.cfg.symbol} cycle error: {exc!r}")
