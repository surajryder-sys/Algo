"""Configuration for V7-Sentinel's Scalper manager. One ScalperSymbolConfig
...
Safety: enable_trading must be explicitly set true (V7S_SCALPER_{SYMBOL}_
...
_SYMBOL_DEFAULTS: dict[str, dict] = {
    "XAUUSD": dict(
        ...
        risk_reward=1.0,              # "1:1"
        magic_number=26092404,   # V7S -- independent from V6S's 26092201
    ),
}
...
def load_symbol_config(symbol: str) -> ScalperSymbolConfig:
    ...
    the scalar fields overridable via V7S_SCALPER_{SYMBOL}_{FIELD} env vars.
    ...
    prefix = f"V7S_SCALPER_{symbol}_"
