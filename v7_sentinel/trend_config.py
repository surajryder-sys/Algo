"""Configuration for V7-Sentinel's Trend Manager (TM-STR). One
...
live on the same MT5 account, so V7S must never share a magic number
...
_SYMBOL_DEFAULTS: dict[str, dict] = {
    "XAUUSD": dict(
        ...
        magic_number=26092401,   # V7S -- independent from V6S's 26091803, see V7S consolidation plan
        bias_timeframe=15,
...
def load_symbol_config(symbol: str) -> TMSymbolConfig:
    ...
    scalar fields overridable via V7S_TM_{SYMBOL}_{FIELD} env vars (e.g.
    V7S_TM_XAUUSD_LOTS). Raises KeyError for a symbol with no tuned
