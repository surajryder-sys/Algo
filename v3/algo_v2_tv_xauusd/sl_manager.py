...
detection. Identical to algo_v2/sl_manager.py -- copied verbatim, no
imports of ob_bridge/atr_bridge/tv_bridge, so nothing needed to change.
See that module's docstring for the full mechanics and worked examples.
"""
...
def _differs(a: Optional[float], b: Optional[float]) -> bool:
    ...
    on the same one. Handles None (no value at all, e.g. no OB edge) as a
    real difference from any actual number, but not from another None."""
