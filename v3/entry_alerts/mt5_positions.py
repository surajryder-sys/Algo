"""Thin, read-only MT5 connection + open-position query -- v3's own
small copy of v3/profit_alerts/mt5_positions.py's identical shape
(same connect pattern, same auto-reconnect fix -- see that module's own
docstring for the confirmed-live IPC-drop incident this protects
against). Kept as its own copy rather than importing profit_alerts's
version directly -- each bot in this repo owns its own MT5 plumbing
(see CLAUDE.md).
...
from v3.entry_alerts.config import Config
...
def get_positions(symbol: str, magic: int) -> list:
    ...
    try:
        ...
        print(f"[entry_alerts] MT5 connection appears broken ({exc}), attempting reconnect...")
        try:
            mt5.shutdown()
            _do_initialize(_cfg)
        except Exception as reconnect_exc:
            print(f"[entry_alerts] MT5 reconnect FAILED: {reconnect_exc}")
            raise
        print("[entry_alerts] MT5 reconnected successfully")
        return _get_positions_once(symbol, magic)
