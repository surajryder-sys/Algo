...
it useless for detecting a manual cancellation (confirmed live on the
XAUUSD bot: a user's own manual cancellations showed up as reason=EXPERT).
Pending-order cancellation detection therefore tracks "did the bot itself
just request this specific cancellation" directly instead -- anything that
disappears without the bot having asked for it is treated as manual.
...
from eth_smc.candidates import parse_order_comment
