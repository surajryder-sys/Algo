...
Independent copy for the ETHUSD bot -- the MT5 Common Files folder is
...
XAUUSD bot's ob_bridge/reader.py. That's fine: OBSTATE files are already
namespaced by symbol, so an ETHUSD-attached indicator instance (running in
the separate MT5 terminal at ETH_SMC_MT5_TERMINAL_PATH) never collides with
the XAUUSD ones.
"""
