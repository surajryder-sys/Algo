...
Released three ways:
  ...
  - manually, via `python -m eth_smc.reset_block <M5|M15|M30|all>`
  - manually, via the RESET M5/M15/M30 buttons on the ETHUSD bridge
    indicator's chart (see check_reset_requests() below)

The RESET flag files and status file are namespaced by symbol
(RESET_ETHUSD_<tf>.flag, BLOCK_STATUS_ETHUSD.json) because the MT5 Common
...
collide with the XAUUSD bot's (algo/blocking.py), which uses the older,
unscoped RESET_<tf>.flag / BLOCK_STATUS.json names and is left untouched.
...
from eth_smc.bridge_reader import bridge_root
