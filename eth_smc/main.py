...
from eth_smc.order_log import log_order_attempt
...
def reconcile_duplicate_fills(cfg: Config) -> None:
    ...
    eth_smc's own history of duplicate fills tonight. Any group of open
    ...
    for comment, group in by_comment.items():
        ...
        if cfg.enable_trading:
            for extra in extras:
                broker.close_position(cfg.symbol, extra, cfg.deviation_points, comment="ESM dedup close")
...
def run_once(cfg: Config, store: TradedZoneStore, blocked: BlockedZoneStore, runtime: RuntimeState,
    ...
    if winner.mode == EntryMode.MARKET:
        ...
        if cfg.enable_trading:
            ...
            # Confirmed live (2026-08-02): a market order that returned a
            # non-DONE retcode still resulted in a real fill -- because
            # mark_traded only ran on the "ok" path, the zone stayed
            # eligible and the next poll placed a genuine duplicate. A
            # misleading/stale retcode must never cause a retry. The only
            # cost of marking early on a send that truly did fail is one
            # missed trade on this zone -- far cheaper than a duplicate
            # live order.
