...
//| v2.00 base: single-instance multi-chart OB bridge. Attach to ONE  |
//| chart; it scans every other open chart (same symbol) for each     |
//| configured timeframe by chart ID, so the publisher no longer      |
//| needs to be attached per timeframe. The LuxAlgo OB detector must  |
//| still run on each of those timeframe charts (it draws the         |
//| pineBox rectangles), but those charts can stay open/minimized     |
//| without our indicator. The ATR Trailing Stop math (KeyValue/      |
//| ATRPeriod below) is untouched from the original ATR_Trail.mq5 --  |
//| only the JSON publish step is new.                                |
//+------------------------------------------------------------------+
#property strict
...
input bool   ShowPanel                      = false;   // per-timeframe Comment() panel -- off by default (decluttered)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "OB+ATR Bridge");
