//+------------------------------------------------------------------+
//|                    OB_StatePublisher_FX.mq5                      |
//| Reads OB rectangles from chart objects, classifies direction,     |
//| calculates virgin status, and publishes latest OB bias/levels.    |
//| FX variant of OB_StatePublisher_Indicator_v2.00: identical        |
//| detection/publish engine, re-defaulted for algo_v2_fx's 9 cross   |
//| pairs -- H1 only (that bot never reads any other timeframe), and  |
//| every on-chart visual (panel/bias labels/virgin-OB list/reset     |
//| buttons) defaulted OFF, since the reset buttons in particular tie |
//| into the M1/M3/M5 BlockedZoneStore flow algo_v2_fx doesn't have.  |
//| Symbol-agnostic (BridgeSymbol="" auto-detects from the chart), so |
//| the same compiled .ex5 gets attached separately to each pair's    |
//| own H1 chart -- one instance per symbol, same as v2.00 needs one  |
//| instance per symbol already.                                      |
//+------------------------------------------------------------------+
#property strict
...
input string TargetTimeframes               = "H1";
...
input bool   ShowBiasLabels                 = false;
...
input bool   ShowVirginObList               = false;   // untested-only OB list per timeframe, under the bias labels
input int    VirginObListYGap               = 10;

input bool   ShowResetButtons               = false;   // RESET M1/M3/M5 buttons -> write a flag file the Python bot polls -- N/A for algo_v2_fx (no BlockedZoneStore), off by default
...
{
   string text = "OB STATE BRIDGE | " + symbol + " | " + IntegerToString(ArraySize(g_targets)) + " timeframes\n\n";
