...
//| (attach to the M5 USOIL chart), two JSON bridge outputs:          |
...
//| Dedicated to USOIL (BridgeSymbol default below); algo_v2_usoil is |
//| the only consumer wired up to these bridge files today. Forked    |
//| from OB_State_XAUUSD_2.0.mq5 -- same engine, but the RESET V2     |
//| button/status row below is trimmed to M5 only (algo_v2_usoil is a |
//| single-tier M5 bot for now; M15/M30 to follow), and its status/   |
//| reset filenames are namespaced "_USOIL" so this indicator and the |
...
input bool   ShowResetButtons               = true;   // RESET M5 button -> writes flag file algo_v2_usoil polls
...
void OnDeinit(const int reason)
{
   ...
   ObjectDelete(0, "OBSP_BLOCKSTATUS_V2_M15");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   string tf = "";
   if(sparam == "OBSP_RESET_V2_M5")       tf = "M5";
   else if(sparam == "OBSP_RESET_V2_M15") tf = "M15";
   else return;
...
void ScanAndPublishAll()
{
   ...
   RepositionV2ResetButtons(status_y + 2 * BiasLabelRowHeight, reset_x);
...
void CreateV2ResetButtons()
{
   ...
   // M5+M15 for now -- an M30 button gets added here once that tier lands.
   CreateResetButton("OBSP_RESET_V2_M5", "RESET M5", ResetButtonX, BiasLabelYStart);
   CreateResetButton("OBSP_RESET_V2_M15", "RESET M15", ResetButtonX + 87, BiasLabelYStart);
}
...
void UpdateV2BlockStatusLabels(const int y, const int x)
{
   ...
   for(int i = 0; i < 2; i++)
   {
      ...
      // One row per timeframe (not side-by-side) -- "M5: BLOCKED
      // (manual_cancel)" is too wide to fit multiple across without
...
void RepositionV2ResetButtons(const int y, const int x)
{
   if(!ShowResetButtons)
      return;
   ObjectSetInteger(0, "OBSP_RESET_V2_M5", OBJPROP_XDISTANCE, x);
   ObjectSetInteger(0, "OBSP_RESET_V2_M5", OBJPROP_YDISTANCE, y);
   ObjectSetInteger(0, "OBSP_RESET_V2_M15", OBJPROP_XDISTANCE, x + 87);
   ObjectSetInteger(0, "OBSP_RESET_V2_M15", OBJPROP_YDISTANCE, y);
}
...
void DeleteV2ResetButtons()
{
   ...
   ObjectDelete(0, "OBSP_RESET_V2_M15");
}
