//+------------------------------------------------------------------+
//|   ATR_Trial_Dual_SuperTrend_Major_Minor.mq5                       |
//|   Merge of ATRTrailDual_MajorMinor.mq5 (ATR Trailing Stop Dual +  |
//|   ZigZag-based Major/Minor swing structure S/R rays) with the      |
//|   standalone Supertrend.mq5 indicator (Pine v4 "Supertrend" port),|
//|   combined into one chart attachment.                              |
//|                                                                    |
//|   NEITHER the ATR Trail Dual block NOR the Major/Minor block below|
//|   had any of their own logic touched -- every function, buffer,   |
//|   and input from ATRTrailDual_MajorMinor.mq5 is carried over      |
//|   verbatim. Only the SUPERTREND section (clearly marked below) is |
//|   new, ported unchanged from Supertrend.mq5, and wired in the same|
//|   way Major/Minor was originally folded into the Dual indicator:  |
//|   its own EnableSupertrend on/off switch, its own ST_-prefixed     |
//|   inputs so nothing collides with the Dual/Major-Minor inputs,     |
//|   own buffers, own ATR handle, own bridge publish.                 |
//|                                                                    |
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "ATR Trail Dual + Supertrend + Major/Minor");
