...
int OnInit()
  {
   ...
   // multi-indicator load on one shared terminal (this project's own setup -- many charts,
   // many indicators, one terminal). Real incident 2026-09-23: a fresh M5 CISD/ATR bar
   // published ~520s late for exactly this reason, so a trade filled far from that bar's own
   // open/close. This 250ms timer is a cheap, periodic nudge -- see OnTimer() below -- that
   // pulls fresh rates independently and re-runs the SAME OnCalculate() already above, so
...
//| never trips a full state reset -- only prev_calculated<=0 does.  |
//+------------------------------------------------------------------+
void OnTimer()
{
