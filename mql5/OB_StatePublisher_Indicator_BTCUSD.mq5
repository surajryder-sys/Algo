//+------------------------------------------------------------------+
//|            OB_StatePublisher_Indicator_BTCUSD.mq5                |
//| BTCUSD-specific copy of OB_StatePublisher_Indicator_v2.00.mq5,   |
//| for the btc_smc Python bot. Same single-instance multi-chart     |
...
//|                                                                    |
//| Why a separate file instead of editing v2.00 (or the ETHUSD copy) |
//| in place: the MT5 Common Files bridge folder (where all these     |
//| indicators publish/read RESET flags) is shared across every       |
//| terminal install for this Windows user, so an unscoped or         |
//| mis-scoped BTCUSD instance's flag files could collide with the    |
//| already-running XAUUSD or ETHUSD instances (all three use M5).    |
//| This file runs in its own MT5 terminal (MetaTrader5-4) and never  |
...
{
   string text = "OB STATE BRIDGE (BTCUSD) | " + symbol + " | " + IntegerToString(ArraySize(g_targets)) + " timeframes\n\n";
