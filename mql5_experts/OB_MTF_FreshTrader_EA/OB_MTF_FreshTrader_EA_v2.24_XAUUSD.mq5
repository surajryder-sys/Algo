...
//| v2.24: newest valid setup across M5/M3/M1 + exact-zone blocks   |
//+------------------------------------------------------------------+
#property strict
...
input double InpHTFMarketDistanceMax   = 4.00;  // Fresh detection distance below this = market entry
input double InpHTFPullbackDistanceMin = 7.00;  // Fresh detection distance must be above this for pullback
input double InpHTFPullbackDistanceMax = 14.00; // Fresh detection distance up to this = pullback entry
...
void RefreshActivePendingSnapshot()
{
   ...
   G_ActivePendingHash=MetaHash();
   G_ActivePendingTFMinutes=MetaTFMinutes();
}
...
bool SendMarket(const TradeSetup &setup)
{
   ...
   const double current_distance=CurrentHTFDistance(source_ob);
   if(current_distance<0.0 || current_distance>=InpHTFMarketDistanceMax)
   {
      Print("Market setup rejected: current price is no longer within the HTF market-entry range. Current distance=",
            DoubleToString(current_distance,_Digits)," | Maximum=",
            DoubleToString(InpHTFMarketDistanceMax,_Digits));
      return false;
   }
...
bool ExecuteSetup(const TradeSetup &setup)
{
   ...
   if(HasPending())
   {
      const uint existing_hash=MetaHash();
      ...
      if(existing_hash!=0 && existing_hash==setup.zone_hash)
         return false;

      // Only a genuinely different and newer setup may replace the order.
      const datetime pending_time=MetaTime();
...
void QueueOrReplace(const TradeSetup &candidate)
{
   ...
   const uint pending_hash=(has_pending?MetaHash():0);
   const datetime pending_time=(has_pending?MetaTime():0);

   // The exact setup already owns the live pending order.
   if(has_pending && pending_hash!=0 && pending_hash==candidate.zone_hash)
      return;
...
string SetupRejectReason(const ENUM_TIMEFRAMES tf,const uint expected_hash)
{
   ...
   if(distance<InpHTFMarketDistanceMax)
   {
      const double current_distance=CurrentHTFDistance(ob);
      if(current_distance<0.0)
         return "current price has crossed beyond the HTF OB boundary";
      if(current_distance>=InpHTFMarketDistanceMax)
         return "current price is too far from the HTF zone for its frozen market entry";
      return "HTF market setup is eligible but was not queued";
   }
...
void DrawPanel()
{
   ...
   Comment("OB MTF FRESH TRADER v2.24\n",
