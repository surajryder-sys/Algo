...
connection (see btc_smc/config.py), separate state/block files, separate
symbol-scoped bridge control files (btc_smc/blocking.py) -- safe to run
...
Safety: BTC_SMC_ENABLE_TRADING must be explicitly set to true in .env for
...
from btc_smc.config import Config, load_config
from btc_smc.entries import EntryMode
from btc_smc.instance_lock import SingleInstanceLock
from btc_smc.intervention import check_manual_pending_cancellations, check_manual_position_closes
from btc_smc.management import compute_trailing_sl, bias_flip_exit_direction
from btc_smc.order_log import log_order_attempt
from btc_smc.state_store import TradedZoneStore
...
def run_once(cfg: Config, store: TradedZoneStore, blocked: BlockedZoneStore, runtime: RuntimeState,
    ...
    # 1. Any bias direction (STRONG, MEDIUM, or SHORTTERM) unconditionally
    #    forces the opposite direction closed/cancelled -- only one position
    #    is ever meant to be open at a time.
    ...
    if winner.mode == EntryMode.MARKET:
        ...
        if cfg.enable_trading:
            ...
            # Confirmed live (2026-08-02, ETHUSD): a market order that
            # returned a non-DONE retcode still resulted in a real fill --
            # because mark_traded only ran on the "ok" path, the zone
            # stayed eligible and the next poll placed a genuine duplicate.
            # A misleading/stale retcode must never cause a retry. The
            # only cost of marking early on a send that truly did fail is
            # one missed trade on this zone -- far cheaper than a
...
def main() -> None:
    ...
    lock = SingleInstanceLock("btc_smc_instance.lock")
    try:
        lock.acquire()
    ...
    print(f"BTC SMC bot starting | symbol={cfg.symbol} lots={cfg.lots} magic={cfg.magic_number} "
