...
CLOSING instead of OPENING, and on a single, much faster confirmation
timeframe (M1 only, not M3/M5): "whenever price touches, and m1 creates an
...
bearish signal (resistance touch + bearish M1 CISD) closes BUY positions, a
confirmed bullish signal (support touch + bullish M1 CISD) closes SELL
positions -- across every manager (TM-STR/RM-STR/RM-ICT alike), same
...
def _close_position(cfg: "ExitManagerSymbolConfig", position, source: "WatchedSource",
                    level_tf: int, level_source: str, cisd_direction: str, confirm_time: int) -> None:
    ...
    print(f"[V6S-XM-LTF] closing {source.name} #{position.ticket} ({_DIR_LABEL[direction]}) -- "
          f"M{level_tf}/{level_source} touch + fresh M1 {cisd_direction} CISD confirmed after it opened")
    detail = {"rule": "HTF touch + M1 CISD (LTF exit)", "level_timeframe": level_tf,
              "level_source": level_source, "cisd": cisd_direction, "cisd_confirm_time": confirm_time,
...
def run_once(cfg: "ExitManagerSymbolConfig", rt: LTFExitRuntime) -> None:
    ...
    cisd = cisd_bridge.fresh_cisd(cfg.symbol, _CISD_TIMEFRAME)
    if cisd is None:
        return
    ...
    confirm_time = cisd.bar_time + _CISD_TIMEFRAME * 60  # real close time of the confirming M1 candle
