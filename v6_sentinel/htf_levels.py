"""HTF (Daily through M15) structure/levels store for RM-STR. Ported
from v5_sentinel/htf_levels.py (2026-09-18). This module only builds the
DATA layer: per-timeframe Strong/Weak/Trap classification, and each
timeframe's own two ATR trail line values exposed as individual price
levels. Touch detection, LTF confirmation (M5/M3/M1), and trade
execution are separate, later pieces (reversal_entry.py).

Timeframe list: D1, H8, H6, H4, H3, H2, H1, M30, M15. H7/H5/M45 were also
considered but dropped -- not standard MT5 timeframes (no
TIMEFRAME_H7/H5/M45 constant exists), and building them as
custom-resampled bars was explicitly declined in V5S's own design in
favor of just dropping them, since there'd be no live MT5 chart to ever
cross-check a custom timeframe against anyway.

Classification and level roles reuse flip_state.py exactly -- same
geometric test the bridge chain already relies on, just reframed here as
levels instead of a live bias:
...
resistance, line below is support," and it holds correctly in all three
character states above without needing the near/far indirection at all.

"Only one trade per flip, resets at next flip": eligibility to trade a
timeframe's current levels is tracked per timeframe, keyed to that
timeframe's own "character_event_time" -- the bar_time its OWN
flip_state last produced a FLIP/TRAP_RESOLVED (i.e. its Strong/Weak/Trap
character actually changing on ITS OWN bar close), NOT a level merely
being touched by price. LevelEligibilityStore persists this to disk
(JSON, keyed by timeframe) so a bot restart doesn't reopen eligibility
that had already been consumed, and resets automatically the moment
that timeframe's own character changes again.

Multi-instrument note (2026-09-18, V6S): LevelEligibilityStore's _state
dict is keyed by tf_minutes ONLY, not (symbol, tf_minutes) -- same
...
# D1, H8, H6, H4, H3, H2, H1, M30, M15 -- in this order.
...
class LevelEligibilityStore:
    ...
    close" design. A bot restart reloads this from disk rather than
    reopening eligibility that was already consumed mid-session, or
    forgetting a touch that armed a level before the restart.
    ...
    def _entry(self, tf_minutes: int) -> dict:
        ...
        persisted under an older schema survives a restart with no
        "traded_directions"/"touched_lines" key at all, and would
        KeyError the moment mark_traded/mark_touched index it directly.
        The setdefault() calls below backfill it lazily instead of
        requiring a clean wipe of the whole state file."""
    ...
    def sync(self, tf_minutes: int, character_event_time: int) -> bool:
        ...
        deliberately has no bridge tie-breaker, see module docstring)
        can still be a path-dependent recompute that occasionally
        computes a transiently different character_event_time on one
        cycle and reverts the next -- if that happens, the
        reset-then-revert would silently wipe traded_directions in
        between, re-arming a direction that had already legitimately
        fired. Rejecting any "new" event_time that's OLDER than what's
        stored closes that gap: a real character change can never look
        like a regression, so anything that does is treated as a
        transient recompute glitch, not trusted."""
        ...
        if stored_event_time is not None and character_event_time < stored_event_time:
            print(f"[V6S-STR-ELIGIBILITY] M{tf_minutes} character_event_time regressed "
        ...
        if entry is None or stored_event_time != character_event_time:
            print(f"[V6S-STR-ELIGIBILITY] M{tf_minutes} character reset "
    ...
    def mark_touched(self, tf_minutes: int, line_no: int, value: float, role: str) -> None:
        ...
        trap never sets last_event), so a touch recorded against the OLD
        value/role must not silently keep counting against whatever that
        slot means now. If the level moved on, the old touch simply no
        longer counts, self-invalidating with no separate reset step
        needed."""
        entry = self._entry(tf_minutes)
