...
from v6_sentinel import broker, telegram_alerts
from v6_sentinel.nlb_nsb_block import BlockStore

if TYPE_CHECKING:
    from v6_sentinel.exit_manager_config import ExitManagerSymbolConfig
...
def run_once(cfg: "ExitManagerSymbolConfig", rt: ZoneTouchAlertRuntime) -> None:
    ...
    for zone in block.zones():
        ...
        text = (f"[V6S] {cfg.symbol}: price touched {zone.timeframe_name} OB zone "
                f"{zone.btm:.3f}-{zone.top:.3f} ({side}), virgin, while a trade is open.")
        print(f"[V6S-ZONE-ALERT] {text}")
