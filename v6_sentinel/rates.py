...
import from algo_v2/v3/v4/v5_sentinel -- this is V6-Sentinel's own copy,
ported verbatim from v5_sentinel/rates.py (2026-09-18) since this module
is already fully symbol-parametrized and stateless -- no change needed
for V6S's multi-instrument design.
...
class TrailSeries:
    ...
    the latest values. opens/highs/lows: candle-color checks (close vs
    open) and swing low/high SL lookback both need them -- harmless
    additive fields for every caller, which only ever reads
    .closes/.trail*/.trend*."""
    symbol: str
    timeframe_minutes: int
    times: list[int]
...
def trail_values_at(series: TrailSeries, bar_time: int) -> Optional[tuple[float, float]]:
    """(trail1, trail2) at a SPECIFIC historical closed bar -- the bridge
    only ever holds a live snapshot (no history endpoint at all), so
    freezing a reference to "the near/far line as of the confirming
    parent's own flip bar" can only come from copy_rates. None if that
    bar_time isn't in the fetched window, or either line was still
    warming up then."""
    try:
...
def recent_swing_low(series: TrailSeries, lookback: int = 8) -> Optional[float]:
    """Lowest LOW of the last `lookback` closed bars -- simple lookback-
    low, no pivot-shape requirement, Major/Minor explicitly NOT used
    here."""
