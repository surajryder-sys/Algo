...
def run_once(cfg: "ExitManagerSymbolConfig") -> None:
    _run(cfg, cfg.sources, CISD_TIMEFRAMES, log_tag="V6S-XM-BIAS", reason="BIASEXIT", comment="V6S-XM-BIAS-SQ",
...
def run_once_scalper(cfg: "ExitManagerSymbolConfig") -> None:
    ...
    if scalper_source is None:
        return
    _run(cfg, [scalper_source], SCALPER_CISD_TIMEFRAMES, log_tag="V6S-XM-SC", reason="SCALPEREXIT",
        comment="V6S-XM-SC-CISD-SQ", rule_desc="opposite fresh M3/M5 CISD (scalper-only exit)")
