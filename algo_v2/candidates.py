...
V2 note: candidate construction itself is unchanged from V1 -- the new
Order Block + ATR Trail zone eligibility filter is applied by the caller
(algo_v2/main.py), not here, using each TradeCandidate's event_time.
"""
...
from algo_v2.entries import (
    EntryMode, EntryPlan, m1_entry_price, m3_entry, m5_entry, select_sl,
)

# "V2" keeps this bot's orders visually distinct from the V1 (algo/) bot's
# "SMC"-prefixed comments while both run on the same terminal/account.
COMMENT_PREFIX = "V2"
...
# "V2|" + 1 tf code + 1 direction code + 6 base36 time digits = 11 chars,
...
def _event_time(zone: Zone) -> int:
    # start_time (the OB's origin candle) is the stable identity -- confirmed
    # live that the bridge's detected_time can flip between two adjacent
    # seconds for the SAME zone (same start_time/high/low) on consecutive
    # scans, which made zone_key churn every poll and caused a cancel/replace
    # loop even though nothing about the setup had actually changed. V1
    # (algo/candidates.py) still prefers detected_time and hasn't shown this
    # symptom, so this fix is scoped to V2 only rather than touching shared
    # code or the MQL5 bridge.
