...
Identical to algo_v2/candidates.py except: imports from this bot's own
reader.py (OBSnapshot/Zone) instead of ob_bridge.reader, and COMMENT_PREFIX
is "TVX" instead of "V2" so this bot's orders are visually and
programmatically distinct from both the V1 ("SMC") and V2 ("V2") bots
sharing the same account/symbol.
...
# broker (confirmed on the V2 bot -- see algo_v2/candidates.py). "TVX|" + 1
# tf code + 1 direction code + 6 base36 time digits = 12 chars, still safely
# under that limit.
...
def _event_time(zone: Zone) -> int:
    # start_time (the OB's origin candle) is the stable identity -- same
    # reasoning as algo_v2/candidates.py: detected_time can jitter for the
    # same zone poll-to-poll, which would make zone_key churn.
...
def build_m1_candidate(direction: int, m1: Optional[OBSnapshot],
    ...
    # direction. An opposite-direction zone sitting in between breaks the
    # sequence. Only the latest zone's virgin status is checked.
